...
public class SOSOrderedHashtable extends Hashtable {
     ...
     * Neither the key nor the value can be null. If the key already exists then
     * the ordering is not changed. If it does not exists then it is added at
     * the end of the OrderedHastable.
     *
     * @param key - the key.
    ...
    /** Returns an Enumeration to enumerate through the keys of the
     * OrderedHashtable. Enumeration will occur in the same order as the keys
     * were put in into the OrderedHashtable. */
