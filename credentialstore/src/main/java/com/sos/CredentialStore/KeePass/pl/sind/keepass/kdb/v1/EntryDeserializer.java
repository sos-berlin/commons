...
public class EntryDeserializer {
    ...
    public void readField(final short fieldType, final int fieldSize, final ByteBuffer data) throws UnsupportedEncodingException {
        ...
        case 0x0000: // Invalid or comment block
            ...
            break;
        case EntryFieldTypes.UUID:  // 0x0001: // UUID, uniquely identifying an
                                   // entry, FIELDSIZE must be 16
            uuid = new UUIDField(fieldType, fieldSize, data);
            break;
        case EntryFieldTypes.GROUP_ID:  // 0x0002: // Group ID, identifying the
                                       // group of the entry, FIELDSIZE = 4; It
                                       // can be any 32-bit value except 0 and
                                       // 0xFFFFFFFF
            groupId = new IdField(fieldType, fieldSize, data);
            break;
        case 0x0003: // Image ID, identifying the image/icon of the entry,
                     // FIELDSIZE = 4
        ...
        case 0x0008: // Notes string, FIELDDATA is an UTF-8 encoded string
            ...
            break;
        case 0x0009: // Creation time, FIELDSIZE = 5, FIELDDATA = packed
                     // date/time
            creationTime = new DateField(fieldType, fieldSize, data);
            break;
        case 0x000A: // Last modification time, FIELDSIZE = 5, FIELDDATA =
                     // packed date/time
            lastModificationTime = new DateField(fieldType, fieldSize, data);
            break;
        case 0x000B: // Last access time, FIELDSIZE = 5, FIELDDATA = packed
                     // date/time
            lastAccessTime = new DateField(fieldType, fieldSize, data);
            break;
        case 0x000C: // Expiration time, FIELDSIZE = 5, FIELDDATA = packed
                     // date/time
    ...
    public Entry getEntry() {
        Entry toReturn = new Entry(uuid, groupId, imageId, title, url, username, password, notes, creationTime, lastModificationTime, lastAccessTime, expirationTime, binaryDescription, binaryData, unknowns, comments);
        reset();
        return toReturn;
    }
